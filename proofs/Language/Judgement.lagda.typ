...
infix  3  _⊢_∙_∶_⊣_  _⊩_∙_∶_⊣_  _⊩→_∙_of_as_∶_⊣_  -- Note: Fix precedence (higher than ∃ 2, lower than ...)

data _⊢_∙_∶_⊣_ : Context → Quantity → Expression → Type → Context → Set
data _⊩_∙_∶_⊣_ : ∀{n} → Context → Quantity ^ n → Expression ^ n → Type ^ n → Context → Set
data _⊩→_∙_of_as_∶_⊣_ : ∀{m} → Context → Quantity → Quantity → Branch ^ m → Type → Type → Context → Set
...
data _⊩→_∙_of_as_∶_⊣_ where
  ...
  ⊩→rest : ∀{Δ Γ₀ Γ₁ Γₘ₊₁ m n q q₀ C₀ e₀ τ₀ xⁿ τⁿ C⟪xⁿ⟫↦eᵐ τ} →
    ...
    Γ₀ ⊩→ q ∙ q₀ of C⟪xⁿ⟫↦eᵐ as τ₀ ∶ τ ⊣ Γₘ₊₁ → -- Note: we're using the same Γ₀ and q₀ here!
    -------------------------------------------------------------------------
    Γ₀ ⊩→ q ∙ q₀ of C₀ ⟪ n × xⁿ ⟫↦ e₀ ∷ C⟪xⁿ⟫↦eᵐ as τ₀ ∶ τ ⊣ Γ₁ ∩ Γₘ₊₁ -- Todo: is ∩ enough here?
```
